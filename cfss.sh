...
do
	...
	do
		...
		do
			...
			if [ $n -ne 0 ]
			then
				echo "$(date +'%H:%M:%S') Wait for Ping Test,Remaining $n"
			else
				echo "$(date +'%H:%M:%S') Ping Test Completed"
				break
			fi
			sleep 1
		done
		...
		if [ $n == 0 ]
		then
			echo "All IPs currently have packet loss"
			echo "Continue with new latency(Ping) Test"
		else
			cat rtt/*.log > rtt.txt
			status=0
			echo "Starting The IP Speed Test"
			cat rtt.txt | sort | awk '{print $2" Latency "$1" ms"}'
			for i in `cat rtt.txt | sort | awk '{print $1"_"$2}'`
			do
				avgms=$(echo $i | awk -F_ '{print $1}')
				ip=$(echo $i | awk -F_ '{print $2}')
				echo "Testing in progress $ip"
				if [ $tls == 1 ]
				then
